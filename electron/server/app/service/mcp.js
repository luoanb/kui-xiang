...
class McpService extends Service {
  ...
  async ensureInitialized() {
    try {
      console.log('[MCP] ensureInitialized called; initialized=', this.initialized)
    } catch (e) {}
    if (!this.initialized) {
  ...
  async initByConfig() {
    ...
    try {
      console.log('[MCP] initByConfig servers:', Object.keys(config || {}))
    } catch (e) {}
    for (const key in config) {
      const serverConfig = config[key]
      // 仅在配置开启时才自动连接（保持已有配置不变）
      if (serverConfig && serverConfig.enabled === false) {
        this.ctx.logger.info(`MCP ${key} 配置为 disabled，跳过自动启动`)
        continue
      }
  ...
  async connectServer(key, serverConfig, errorCallback) {
    try {
      console.log(`[MCP] connectServer called for ${key}; serverConfig.enabled= ${serverConfig && serverConfig.enabled}`)
  ...
  async getConfig() {
    try {
      ...
      // 补充 enabled 字段（如果不存在则默认启用，保持已有配置不变）
      let modified = false
      for (const k of Object.keys(servers)) {
        if (typeof servers[k].enabled === 'undefined') {
          servers[k].enabled = true
          modified = true
        }
      }

      // 如果我们对读取到的配置做了默认值补充，则写回配置文件以持久化
      if (modified) {
        configJSON.mcpServers = servers
        try {
          fs.writeFileSync(configFile, JSON.stringify(configJSON, null, 2), 'utf8')
          this.ctx.logger.info('[MCP]已为缺失的配置项补充 enabled 字段并保存配置文件')
        } catch (err) {
          this.ctx.logger.error('[MCP]写回配置文件失败:', err)
        }
      }
  ...
  async addServer(serverData) {
    ...
    try {
      ...
      for (const [key, server] of Object.entries(serverData)) {
        ...
        // 补充 enabled 字段，保持已有值
        if (typeof serverConfig.enabled === 'undefined') {
      ...
      for (const [key, server] of Object.entries(serverData)) {
        // 移除readme相关字段
        const { readme, readmeCN, ...serverConfig } = server
        // 仅在 enabled 未显式为 false 时连接
        if (serverConfig && serverConfig.enabled === false) {
          this.ctx.logger.info(`添加的 MCP ${key} 配置为 disabled，跳过启动`)
        } else {
          await this.connectServer(key, serverConfig)
        }
      }
  ...
  async updateServer(serverData) {
    try {
      ...
      try {
        ...
        this.ctx.logger.error(`重启/调整MCP服务器 ${serverKey} 状态失败:`, error)
      }
  ...
  async saveMcpServer(key, serverConfig) {
    ...
    try {
      ...
      // 仅在 enabled 未显式为 false 时连接
      if (serverConfig && serverConfig.enabled === false) {
        this.ctx.logger.info(`保存的 MCP ${key} 配置为 disabled，跳过启动`)
        result = { success: true }
      } else {
        result = await this.connectServer(key, serverConfig)
      }
