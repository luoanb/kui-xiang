...
const downloadTool = async (tool) => {
  try {
    ...
    await checkTool(tool.name)
    // 检查是否安装成功
    const toolState = tools.value.find(t => t.name === tool.name)
    if (toolState?.installed) {
      toast({
        title: t('mcp.env.messages.downloadSuccess'),
        description: t('mcp.env.messages.downloadSuccessDesc', { tool: tool.name }),
      })
    }
  } catch (error) {
    console.error(`下载工具 ${tool.name} 失败:`, error)
    toast({
      title: t('mcp.env.messages.downloadFailed'),
      description: error?.message || t('mcp.env.messages.downloadFailedDesc', { tool: tool.name }),
      variant: "destructive",
    })
  } finally {
