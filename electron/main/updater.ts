...
export class AppUpdater {
  ...
  private async initUpdate() {
    ...
    for (const server of serverList) {
      try {
        console.log('[updater_ts]', `尝试设置更新服务器：${server.name} (${server.url})`);
        ...
        console.log('[updater_ts]', `✅ 成功使用更新服务器：${server.name}, URL: ${server.url}`);
        break;
      } catch (e) {
  ...
  private bindEvents() {
    ...
    autoUpdater.on('update-not-available', (info) => {
      console.log('[updater_ts]', '当前已是最新版本:', info)
    ...
    autoUpdater.on('update-downloaded', (info) => {
      console.log('[updater_ts]', '更新已下载:', info)
      if (this.mainWindow) {
  ...
  public async installUpdate(): Promise<boolean> {
    try {
      ...
      if (response === 0) {
        console.log('[updater_ts]', '用户选择立即安装更新')
        setImmediate(() => {
        ...
        console.log('[updater_ts]', '用户选择稍后安装')
        return false
      }
    } catch (err) {
...
export function registerUpdaterHandlers(updater: AppUpdater) {
  ...
  ipcMain.handle('check-update', async () => {
    try {
      ...
      console.log('[updater_ts]', '检查更新失败:', error)
