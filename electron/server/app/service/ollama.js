...
class OllamaService extends Service {
  ...
  async start() {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        console.log('[ollama_js]', attempts, response)

        if (response.status === 200) {
          return // 服务已启动
        }
      } catch (error) {
        console.log('[ollama_js]', error)
        // 忽略错误,继续重试
      }
  ...
  async pullModel(modelName) {
    ...
    try {
      ...
      console.log('[ollama_js]', '模型安装失败:', error)
      ctx.res.end()
