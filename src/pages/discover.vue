  ...
  if (cachedData && cachedTimestamp) {
    ...
    if (now - timestamp < CACHE_EXPIRY) {
      try {
        ...
        console.log('[discover_vue]', `成功从缓存加载数据 (${lang})`)
        isLoading.value = false
        // 在后台更新缓存
...
const refreshCacheInBackground = async (lang: string) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('[discover_vue]', `后台更新缓存成功 (${lang})`)
    }
  } catch (error) {
...
const loadData = async (lang: string) => {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      dataList.value = data
      console.log('[discover_vue]', `成功从远程加载数据 (${lang})`)

      // 缓存数据
      localStorage.setItem(currentCacheKey, JSON.stringify(data))
      localStorage.setItem(currentCacheTimestampKey, Date.now().toString())
      console.log('[discover_vue]', `数据已缓存到本地 (${lang})`)
    } else {
    ...
    try {
      ...
      console.log('[discover_vue]', `成功从本地文件加载数据 (${lang})`)
    } catch (localError) {
  ...
  try {
    ...
    if (cachedData) {
      try {
        ...
        console.log('[discover_vue]', `使用缓存数据进行过滤 (${lang})`)
      } catch (error) {
        console.error(`解析缓存数据失败 (${lang}):`, error)
